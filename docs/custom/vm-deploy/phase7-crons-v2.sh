...
add "monday-weigh-in" \
  --name "monday-weigh-in" --cron "45 5 * * 1" --tz "$TZ_SAST" \
  --session isolated --announce --channel whatsapp --to "$TO" \
  --message "Monday weigh-in. Step on the scale before eating. Reply: I weigh X. After Henzard replies, log via sparky_fitness weight [X]. Then load the health-coach skill and read the Weight Goal section. Calculate: current weight vs this month's milestone target. Calculate weekly delta from last Monday's weight in MEMORY.md under the Weight Log section. Respond per the Weight Response Rules in the skill. Then append silently to ~/.openclaw/workspace/MEMORY.md under ## Weight Log (create if missing): YYYY-MM-DD | [X]kg | target: [monthly target]kg | delta: [+/- from last week]kg | status: [on-track/behind/ahead]. Do not announce the log."

add "wednesday-weight-nudge" \
  --name "wednesday-weight-nudge" --cron "0 14 * * 3" --tz "$TZ_SAST" \
  --session isolated --announce --channel whatsapp --to "$TO" \
  --message "Mid-week weight check. Read ~/.openclaw/workspace/MEMORY.md under ## Weight Log. Find the most recent Monday entry. If no entry exists for this Monday, send: 'No weigh-in logged this week. Step on the scale tomorrow morning and reply: I weigh X.' If an entry exists, check the status field. If status is behind, load the health-coach skill and read the Weight Goal section. Suggest ONE structural fix from the skill. If status is on-track or ahead, say nothing — do not send this message at all."
